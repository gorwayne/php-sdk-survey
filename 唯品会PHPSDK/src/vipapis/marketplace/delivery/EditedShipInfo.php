...
class EditedShipInfo {
	...
	public $edited_package_list = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['edited_package_list'])){
				
				$this->edited_package_list = $vals['edited_package_list'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("edited_package_list" == $schemeField){
				
				$needSkip = false;
				
				$this->edited_package_list = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$this->edited_package_list[$_size0++] = $elem0;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('edited_package_list');
		
		if (!is_array($this->edited_package_list)){
			
			throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
		}
		
		$output->writeListBegin();
		foreach ($this->edited_package_list as $iter0){
